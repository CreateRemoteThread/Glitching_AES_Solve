...
class AESModified:
  ...
  def encryptToRound(self,data,round,keyModification,tnum):
    ...
    for rnum in range(0,round):
      # Don't cache the R8 calculation.
...
for lastRoundKey in range(0,256):
  keyModification[TARGET_ROUND-1][10] = lastRoundKey
  argmaxQDiffs = []
  maxQDiffs = []
  lastRoundKey
  ...
  print("LastRoundKey %02x , XOR Key: %s (Pos %d, Max QDiffs %f, Avg QDiffs %f)" % (lastRoundKey, hex(np.argmax(maxQDiffs)), argmaxQDiffs[np.argmax(maxQDiffs)], max(maxQDiffs),np.mean(maxQDiffs)))
