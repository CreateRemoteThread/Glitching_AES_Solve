...
class AESModified:
  def __init__(self,key):
    self.key = key
    self.fwdkey = key_schedule_rounds(self.key,0,10)

  def decryptToRound(self,data,round,keyModification,tnum):
    state = data
    state = [state[bnum] ^ self.fwdkey[bnum] for bnum in range(0,16)]
  ...
  def encryptToRound(self,data,round,keyModification,tnum):
    ...
    for rnum in range(0,round):
      if (rnum != round - 1) and (tnum in aesCache[rnum].keys()):
